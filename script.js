...
let state = {
    ...
    friends: [
        ...
        { id: 'PUG-91AB', name: "PixelPaws", coins: 1420, status: 'studying' },
        { id: 'PUG-22XX', name: "StudyBuddy99", coins: 850, status: 'online' },
        { id: 'PUG-00ZZ', name: "DoggoLover", coins: 430, status: 'offline' }
    ],
    settings: {
        petName: "Pug",
        sound: false,
...
function loadData() {
    try {
        ...
        if (str) {
            ...
            state.friends = parsed.friends || state.friends; // Persists the custom friends list
...
function updateCoinDisplays() {
    ...
    let me = state.friends.find(f => f.id === 'me');
...
// Replaces old direct onclick
timerDisplay.addEventListener('click', () => {
    if (state.timer.isStudying) return; // Disallow modification while running
    ...
    if (timerClickCount === 1) {
    ...
    } else if (timerClickCount === 2) {
        // It's a double click! Switch to fast input edit.
        ...
        // Populate minutes
        timerEditInput.value = Math.floor(state.timer.initialDuration / 60);
...
timerEditInput.addEventListener('keydown', function(e) {
    if (e.key === 'Enter') {
        const val = this.value.trim();

        // VALIDATION: Catch letters (isNaN) or empty box
        if (isNaN(val) || val === '') {
            timerError.classList.remove('hidden');
            
            // Wait 1.5 seconds, then hide error and REVERT to old text display
            setTimeout(() => {
            ...
            // Success: Apply custom time
            let mins = parseInt(val, 10);
            if(mins <= 0) mins = 1; // force minimum 1 min
            
            setDuration(mins); // sets state and updates text natively
...
// Revert if they click off the box without hitting enter
timerEditInput.addEventListener('blur', function() {
    if(!this.classList.contains('hidden') && timerError.classList.contains('hidden')) {
        this.classList.add('hidden');
        timerDisplay.classList.remove('hidden');
    }
});
...
function setDuration(mins) {
    ...
    updateTimerDisplay(); // explicitly update visually
    if (!document.getElementById('duration-picker').classList.contains('hidden')) {
        closeDurationPicker();
    }
}
...
function addFriend() {
    ...
    if (code === "PUG-4X9B") { showToast("That's your own code!"); return; }
    ...
    // Generate a mock friend profile based on code typed
    const newFriend = {
        id: code,
        name: "Friend_" + code.substring(0, 4),
...
function renderFriends() {
    ...
    [...state.friends].sort((a, b) => b.coins - a.coins).forEach((friend, idx) => {
